...
public class CreateCollectionsTest extends AbstractDBTest {
	...
	public void testCreateCollection() {
		try {
			System.out.println("Created Collection: " + root.getName() + "( "
					+ root.getClass() + " )");

			Service[] services = root.getServices();
			System.out.println("services array: " + services);
			assertTrue("Collection must provide at least one Service",
			...
			assertTrue("Collection just created: ChildCollectionCount==0",
					ccc == 0);
			assertTrue("Collection state should be Open after creation",
					testCollection.isOpen());

			String directory = "samples/shakespeare";
			System.out.println("---------------------------------------");
			System.out.println("storing all XML files in directory "
					+ directory + "...");
			System.out.println("---------------------------------------");
			...
			System.out.println("fileNames stored: "
					+ fileNamesJustStored.toString());

			String[] resourcesNames = testCollection.listResources();
			int resourceCount = testCollection.getResourceCount();
			System.out.println("testCollection.getResourceCount()="
					+ resourceCount);
			...
			assertTrue("After removal resource count must decrease",
					testCollection.getResourceCount() == resourceCount - 1);
			// restore the resource just removed :
			storeResourceFromFile(new File(f, fileToRemove), testCollection);

			// byte[] data = storeBinaryResourceFromFile( new File( existDir,
			// "webapp/logo.jpg"), testCollection);
			// Object content =
			// testCollection.getResource("logo.jpg").getContent();
	...
	private XMLResource storeResourceFromFile(File file,
			Collection testCollection) {
		XMLResource res = null;
		try {
			System.out.println("storing " + file.getAbsolutePath());
			String xml;
			res = (XMLResource) testCollection.createResource(file.getName(),
					"XMLResource");
	...
	private byte[] storeBinaryResourceFromFile(File file,
			Collection testCollection) throws Exception {
		byte[] data = null;
		System.out.println("storing " + file.getAbsolutePath());
	...
	public void testMultipleCreates() {
		try {
			Collection rootColl = DatabaseManager
					.getCollection("xmldb:exist://" + DBBroker.ROOT_COLLECTION);
			CollectionManagementService cms = (CollectionManagementService) rootColl
					.getService("CollectionManagementService", "1.0");
			assertNotNull(cms);
