...
public class CollectionConfigurationTest extends AbstractDBTest {

	private final static String URI = "xmldb:exist://";

	private final static String DRIVER = "org.exist.xmldb.DatabaseImpl";

	private final static XmldbURI TEST_COLLECTION = XmldbURI.ROOT_COLLECTION_URI
			.append("testIndexConfiguration");

	private final static XmldbURI COLLECTION_SUB1 = TEST_COLLECTION
			.append("sub1");
	private final static XmldbURI COLLECTION_SUB2 = COLLECTION_SUB1
			.append("sub2");

	private static final XmldbURI CONF_COLL_URI = XmldbURI.CONFIG_COLLECTION_URI
			.append(TEST_COLLECTION);
	private static final XmldbURI CONF_COLL_URI2 = CONF_COLL_URI
			.append(TestConstants.SPECIAL_NAME);
	// private static final String CONF_COLL_PATH =
	...
	private static final XmldbURI TEST_CONFIG_NAME_1 = XmldbURI
			.create("test1.xconf");
	private static final XmldbURI TEST_CONFIG_NAME_2 = XmldbURI
			.create(TestConstants.SPECIAL_NAME.toString() + ".xconf");

	private final static String DOCUMENT_CONTENT = "<test>" + "<a>001</a>"
			+ "<a>01</a>" + "<a>1</a>" + "<b>001</b>" + "<b>01</b>"
			+ "<b>1</b>" + "</test>";

	private final static String DOCUMENT_CONTENT2 = "<test x='0'>"
			+ "<c c='2002-12-07T12:20:46.275+01:00'>2002-12-07T12:20:46.275+01:00</c>"
			+ "<d d='1'>1</d>" + "<e e='1'>1</e>" + "<f f='true'>true</f>"
			+ " <g g='1'>1</g>" + "<h h='1'>1</h>"
			+ "<test x='1'><test x='2'></test></test></test>";

	private final static String DOCUMENT_CONTENT3 = "<test>" + "   <a>1</a>"
			+ "   <b>1</b>" + "   <c>1</c>" + "   <d>x</d>" + "   <e>xx</e>"
			+ "   <f>xxx</f>" + "</test>";
	...
	public void setUp() throws Exception {
		...
		Collection configColl = DatabaseManager.getCollection(URI
				+ CONF_COLL_URI.toString(), "admin", null);
		if (configColl == null) {
			System.out.println("creating collection '" + CONF_COLL_URI + "'");
			CollectionManagementService cms = (CollectionManagementService) testCollection
					.getService("CollectionManagementService", "1.0");
	...
	public void tearDown() throws Exception {
		...
		Collection confCol = DatabaseManager.getCollection(URI + CONF_COLL_URI,
				"admin", null);
		if (confCol != null)
	...
	public void testCollectionConfigurationService1() {
		...
		try {
			...
			// ... but 1 string value
			result = service
					.query("util:index-key-occurrences(/test/b, \"1\")");
			assertEquals("1", result.getResource(0).getContent());

			// 3 numeric values
			result = service
					.query("util:qname-index-lookup(xs:QName(\"a\"), 1 ) ");
			assertEquals(3, result.getSize());
			// ... but 1 string value
			result = service
					.query("util:qname-index-lookup(xs:QName(\"b\"), \"1\" ) ");
	...
	public void testCollectionConfigurationService3() {
		...
		try {
			...
			storeConfiguration(CONF_COLL_URI,
	...
	public void testCollectionConfigurationService5() {
		...
		try {
			// Configure collection *manually*
			XmldbURI configurationFileName = XmldbURI.create("foo"
					+ CollectionConfiguration.COLLECTION_CONFIG_SUFFIX);
			...
			result = service.query("xmldb:get-child-resources('"
					+ CONF_COLL_URI + "')");
			assertEquals(configurationFileName.toString(), result
	...
	public void testCollectionConfigurationService6() {
		...
		try {
			...
			// Adding confMgr.invalidateAll(getName()); in
			// Collection.storeInternal solved the problem
	...
	/**
	 * Check if configurations are properly passed down the collection
	 * hierarchy.
	 */
	@Test
	public void testCollectionConfigurationService7() {
		...
		try {
			...
			// Configure collection automatically
			// sub2 should inherit its index configuration from the top
			// collection
			IndexQueryService idxConf = (IndexQueryService) testCollection
					.getService("IndexQueryService", "1.0");
			idxConf.configureCollection(CONFIG1);
			...
			// ... then index document
			XMLResource doc = (XMLResource) sub2.createResource(
					TestConstants.TEST_XML_URI.toString(), "XMLResource");
			doc.setContent(DOCUMENT_CONTENT);
			sub2.storeResource(doc);

			XPathQueryService service = (XPathQueryService) sub2.getService(
	...
	public void testCollectionConfigurationService8() {
		...
		try {
			...
			// Overwrite main configuration with an empty configuration in the
			// subcollection
			idxConf = (IndexQueryService) sub2.getService("IndexQueryService",
					"1.0");
	...
	public void testCollectionConfigurationService9() {
		...
		try {
			...
			// Overwrite main configuration with an empty configuration in the
			// subcollection
			idxConf = (IndexQueryService) sub1.getService("IndexQueryService",
					"1.0");
			idxConf.configureCollection(EMPTY_CONFIG);
			...
			XMLResource doc = (XMLResource) sub1.createResource(
					TestConstants.TEST_XML_URI.toString(), "XMLResource");
			doc.setContent(DOCUMENT_CONTENT);
			sub1.storeResource(doc);

			XPathQueryService service = (XPathQueryService) sub1.getService(
					"XPathQueryService", "1.0");
			...
			doc = (XMLResource) sub2.createResource(
					TestConstants.TEST_XML_URI.toString(), "XMLResource");
			doc.setContent(DOCUMENT_CONTENT);
			sub2.storeResource(doc);

			service = (XPathQueryService) sub2.getService("XPathQueryService",
					"1.0");
	...
	public void testCollectionConfigurationService10() {
		...
		try {
			...
			Collection confCol = DatabaseManager.getCollection(URI
					+ XmldbURI.CONFIG_COLLECTION_URI.append(TEST_COLLECTION),
					"admin", null);
			...
			// confCol.getService("CollectionManagementService", "1.0");
	...
	public void testCollectionConfigurationService11() {
		...
		try {
			...
			Collection confCol = DatabaseManager.getCollection(URI
					+ XmldbURI.CONFIG_COLLECTION_URI.append(TEST_COLLECTION),
					"admin", null);
			cms = (CollectionManagementService) confCol.getService(
	...
	public void testRangeIndex1() {
		...
		try {
			...
			result = service
					.query("util:index-key-occurrences(/test/c, xs:dateTime(\"2002-12-07T12:20:46.275+01:00\") )");
			assertEquals(1, result.getSize());
			assertEquals("1", result.getResource(0).getContent());
			...
			result = service
					.query("util:index-key-occurrences(/test/d, xs:double(1) )");
			assertEquals(1, result.getSize());
			assertEquals("1", result.getResource(0).getContent());
			...
			result = service
					.query("util:index-key-occurrences(/test/e, xs:float(1) )");
			assertEquals(1, result.getSize());
			assertEquals("1", result.getResource(0).getContent());
			...
			result = service
					.query("util:index-key-occurrences(/test/f, true())");
			assertEquals(1, result.getSize());
			assertEquals("1", result.getResource(0).getContent());
			...
			result = service
					.query("util:index-key-occurrences(/test/g, xs:integer(1))");
			assertEquals(1, result.getSize());
			assertEquals("1", result.getResource(0).getContent());
			...
			result = service
					.query("/test/c[(# exist:force-index-use #) { . = xs:dateTime(\"2002-12-07T12:20:46.275+01:00\") }]");
			assertEquals(1, result.getSize());

			result = service
					.query("/test[(# exist:force-index-use #) { c = xs:dateTime(\"2002-12-07T12:20:46.275+01:00\") }]");
			assertEquals(1, result.getSize());

			result = service
					.query("/test/d[(# exist:force-index-use #) { . = xs:double(1) }]");
			assertEquals(1, result.getSize());

			result = service
					.query("/test[(# exist:force-index-use #) { d = xs:double(1) }]");
			assertEquals(1, result.getSize());

			result = service
					.query("/test/e[(# exist:force-index-use #) { . = xs:float(1) }]");
			assertEquals(1, result.getSize());

			result = service
					.query("/test[(# exist:force-index-use #) { e = xs:float(1) }]");
			assertEquals(1, result.getSize());

			result = service
					.query("/test/f[(# exist:force-index-use #) { . = true() }]");
			assertEquals(1, result.getSize());

			result = service
					.query("/test[(# exist:force-index-use #) { f = true() }]");
			assertEquals(1, result.getSize());

			result = service
					.query("/test/g[(# exist:force-index-use #) { . = 1 }]");
			assertEquals(1, result.getSize());

			result = service
					.query("/test[(# exist:force-index-use #) { g = 1 }]");
			assertEquals(1, result.getSize());

			result = service
					.query("/test/h[(# exist:force-index-use #) { . = '1' }]");
			assertEquals(1, result.getSize());

			result = service
					.query("/test[(# exist:force-index-use #) { h = '1' }]");
			assertEquals(1, result.getSize());

			boolean exceptionCaught = false;
			try {
				result = service
						.query("/test[(# exist:force-index-use #) { contains(d, '1') }]");
				assertEquals(0, result.getSize());
			} catch (XMLDBException e) {
				exceptionCaught = true;
			}
			assertTrue("contains() should not use index of type xs:double",
					exceptionCaught);
			exceptionCaught = false;
			try {
				result = service
						.query("/test[(# exist:force-index-use #) { matches(d, '1') }]");
				assertEquals(0, result.getSize());
			} catch (XMLDBException e) {
				exceptionCaught = true;
			}
			assertTrue("matches() should not use index of type xs:double",
					exceptionCaught);
	...
	public void testRangeIndex2() {
		...
		try {
			...
			result = service
					.query("(# exist:force-index-use #) { /test/c[. = xs:dateTime(\"2002-12-07T12:20:46.275+01:00\")] }");
			assertEquals(1, result.getSize());
			...
			result = service
					.query("(# exist:force-index-use #) { /test/d[. = xs:double(1)] }");
			assertEquals(1, result.getSize());
			...
			result = service
					.query("(# exist:force-index-use #) { /test/e[. = xs:float(1)] }");
			assertEquals(1, result.getSize());
			...
			result = service
					.query("(# exist:force-index-use #) { /test/f[. = true()] }");
			assertEquals(1, result.getSize());
			...
			result = service
					.query("(# exist:force-index-use #) { /test/g[. = 1] }");
			assertEquals(1, result.getSize());
			...
			result = service
					.query("(# exist:force-index-use #) { /test/h[. = '1'] }");
			assertEquals(1, result.getSize());
			...
			try {
				result = service
						.query("(# exist:force-index-use #) { /test[contains(d, '1')] }");
				assertEquals(0, result.getSize());
			} catch (XMLDBException e) {
				exceptionCaught = true;
			}
			...
			try {
				result = service
						.query("(# exist:force-index-use #) { /test[matches(d, '1')] }");
				assertEquals(0, result.getSize());
			} catch (XMLDBException e) {
				exceptionCaught = true;
			}
	...
	public void testRangeIndex3() {
		try {
			...
			XQueryService service = (XQueryService) testCollection.getService(
					"XQueryService", "1.0");
			// the query optimizer cannot optimize the following general
			// comparison as
			// the context qname is unknown. however, the available qname index
			// should still be used.
			ResourceSet result = service
					.query("(# exist:force-index-use #) { for $t in /test/a where $t = 1 return $t}");
			assertEquals(1, result.getSize());
			result = service
					.query("(# exist:force-index-use #) { for $t in /test/d where contains($t, 'x') return $t}");
			assertEquals(1, result.getSize());
			result = service
					.query("(# exist:force-index-use #) { for $t in /test/d where matches($t, 'x$') return $t}");
			assertEquals(1, result.getSize());

			// left operand to comparison uses nodes from different elements,
			// both having an index defined
			// by qname: use qname index
			result = service
					.query("(# exist:force-index-use #) { for $t in /test/(a|b) where $t = 1 return $t}");
			assertEquals(2, result.getSize());
			result = service
					.query("(# exist:force-index-use #) { for $t in /test/(d|e) where contains($t, 'x') return $t}");
			assertEquals(2, result.getSize());
			result = service
					.query("(# exist:force-index-use #) { for $t in /test/(d|e) where matches($t, 'x$') return $t}");
			assertEquals(2, result.getSize());

			// left operand to comparison uses nodes from different elements
			// with mixed indexes,
			// some defined on qname, one defined by path: comparison needs to
			// scan all 3 indexes.
			result = service
					.query("(# exist:force-index-use #) { for $t in /test/(a|b|c) where $t = 1 return $t}");
			assertEquals(3, result.getSize());
			result = service
					.query("(# exist:force-index-use #) { for $t in /test/(d|e|f) where contains($t, 'x') return $t}");
			assertEquals(3, result.getSize());
			result = service
					.query("(# exist:force-index-use #) { for $t in /test/(d|e|f) where matches($t, 'x$') return $t}");
			assertEquals(3, result.getSize());

			// left operand has index defined on path. other elements in the
			// collection use indexes
			// on qname: comparison needs to scan all index types.
			result = service
					.query("(# exist:force-index-use #) { for $t in /test/c where $t = 1 return $t}");
			assertEquals(1, result.getSize());

			// simple comparison, left operand has index defined on path.
			result = service
					.query("(# exist:force-index-use #) { /test[c = 1] }");
			assertEquals(1, result.getSize());
			result = service
					.query("(# exist:force-index-use #) { /test[matches(d, 'x')] }");
			assertEquals(1, result.getSize());
			...
			try {
				result = service
						.query("(# exist:force-index-use #) { for $t in /test/c where contains($t, '1') return $t}");
				assertEquals(1, result.getSize());
			} catch (XMLDBException e) {
				exceptionCaught = true;
			}
			...
			try {
				result = service
						.query("(# exist:force-index-use #) { for $t in /test/c where matches($t, '1') return $t}");
				assertEquals(1, result.getSize());
			} catch (XMLDBException e) {
				exceptionCaught = true;
			}
			...
			try {
				result = service
						.query("(# exist:force-index-use #) { /test[matches(c, '1')] }");
				assertEquals(1, result.getSize());
			} catch (XMLDBException e) {
				exceptionCaught = true;
			}
	...
	public void testRangeIndexOverAttributes() {
		...
		try {
			...
			result = service
					.query("//test[(# exist:force-index-use #) { @x = 0 }]");
			assertEquals(1, result.getSize());

			result = service
					.query("//test[(# exist:force-index-use #) { @x eq 0 }]");
			assertEquals(1, result.getSize());
			...
			result = service
					.query("util:index-key-occurrences(/test/c/@c, xs:dateTime(\"2002-12-07T12:20:46.275+01:00\") )");
			assertEquals(1, result.getSize());
			assertEquals("1", result.getResource(0).getContent());
			...
			result = service
					.query("util:index-key-occurrences(/test/d/@d, xs:double(1) )");
			assertEquals(1, result.getSize());
			assertEquals("1", result.getResource(0).getContent());
			...
			result = service
					.query("util:index-key-occurrences(/test/e/@e, xs:float(1) )");
			assertEquals(1, result.getSize());
			assertEquals("1", result.getResource(0).getContent());
			...
			result = service
					.query("util:index-key-occurrences(/test/f/@f, true())");
			assertEquals(1, result.getSize());
			assertEquals("1", result.getResource(0).getContent());
			...
			result = service
					.query("util:index-key-occurrences(/test/g/@g, xs:integer(1))");
			assertEquals(1, result.getSize());
			assertEquals("1", result.getResource(0).getContent());
			...
			result = service
					.query("util:index-key-occurrences(/test//@h, '1')");
			assertEquals(1, result.getSize());
			assertEquals("1", result.getResource(0).getContent());
			...
			result = service
					.query("util:index-key-occurrences(/test/h/@h, '1')");
			assertEquals(1, result.getSize());
			assertEquals("1", result.getResource(0).getContent());
			...
			result = service
					.query("/test/c/@c[(# exist:force-index-use #) { . = xs:dateTime(\"2002-12-07T12:20:46.275+01:00\") }]");
			assertEquals(1, result.getSize());

			result = service
					.query("/test[(# exist:force-index-use #) { ./c/@c = xs:dateTime(\"2002-12-07T12:20:46.275+01:00\") }]");
			assertEquals(1, result.getSize());
			...
			result = service
					.query("/test/d/@d[(# exist:force-index-use #) { . = xs:double(1) }]");
			assertEquals(1, result.getSize());

			result = service
					.query("/test[(# exist:force-index-use #) { ./d/@d = xs:double(1) }]");
			assertEquals(1, result.getSize());
			...
			result = service
					.query("/test/e/@e[(# exist:force-index-use #) { . = xs:float(1) }]");
			assertEquals(1, result.getSize());

			result = service
					.query("/test[(# exist:force-index-use #) { ./e/@e = xs:float(1) }]");
			assertEquals(1, result.getSize());
			...
			result = service
					.query("/test/f/@f[(# exist:force-index-use #) { . = true() }]");
			assertEquals(1, result.getSize());

			result = service
					.query("/test[(# exist:force-index-use #) { ./f/@f = true() }]");
			assertEquals(1, result.getSize());
			...
			result = service
					.query("/test/g/@g[(# exist:force-index-use #) { . = 1 }]");
			assertEquals(1, result.getSize());

			result = service
					.query("/test[(# exist:force-index-use #) { ./g/@g = 1 }]");
			assertEquals(1, result.getSize());
			...
			result = service
					.query("/test/h/@h[(# exist:force-index-use #) { . = '1' }]");
			assertEquals(1, result.getSize());

			result = service
					.query("/test[(# exist:force-index-use #) { ./h/@h = '1' }]");
			assertEquals(1, result.getSize());
	...
	public void testMultipleConfigurations00() {
		checkStoreConf(CONF_COLL_URI, TEST_CONFIG_NAME_1, CONF_COLL_URI,
				TEST_CONFIG_NAME_1, true);
	}

	@Test
	public void testMultipleConfigurations01() {
		checkStoreConf(CONF_COLL_URI, TEST_CONFIG_NAME_1, CONF_COLL_URI,
				TEST_CONFIG_NAME_2, false);
	}

	@Test
	public void testMultipleConfigurations02() {
		checkStoreConf(CONF_COLL_URI, TEST_CONFIG_NAME_1, CONF_COLL_URI2,
				TEST_CONFIG_NAME_1, true);
	}
	...
	public void testMultipleConfigurations03() {
		checkStoreConf(CONF_COLL_URI, TEST_CONFIG_NAME_1, CONF_COLL_URI2,
				TEST_CONFIG_NAME_2, true);
	}

	@Test
	public void testMultipleConfigurations04() {
		checkStoreConf(CONF_COLL_URI, TEST_CONFIG_NAME_2, CONF_COLL_URI,
				TEST_CONFIG_NAME_1, false);
	}
	...
	public void testMultipleConfigurations06() {
		checkStoreConf(CONF_COLL_URI, TEST_CONFIG_NAME_2, CONF_COLL_URI2,
				TEST_CONFIG_NAME_1, true);
	}
	...
	public void testMultipleConfigurations08() {
		checkStoreConf(CONF_COLL_URI2, TEST_CONFIG_NAME_1, CONF_COLL_URI,
				TEST_CONFIG_NAME_1, true);
	}
	...
	public void testMultipleConfigurations10() {
		checkStoreConf(CONF_COLL_URI2, TEST_CONFIG_NAME_1, CONF_COLL_URI2,
				TEST_CONFIG_NAME_1, true);
	}
	...
	public void testMultipleConfigurations12() {
		checkStoreConf(CONF_COLL_URI2, TEST_CONFIG_NAME_2, CONF_COLL_URI,
				TEST_CONFIG_NAME_1, true);
	}
	...
	public void testMultipleConfigurations14() {
		checkStoreConf(CONF_COLL_URI2, TEST_CONFIG_NAME_2, CONF_COLL_URI2,
				TEST_CONFIG_NAME_1, false);
	}
	...
	private void checkStoreConf(XmldbURI coll1, XmldbURI confName1,
			XmldbURI coll2, XmldbURI confName2, boolean shouldSucceed) {
		try {
			storeConfiguration(coll1, confName1, CONFIG1);
			storeConfiguration(coll2, confName2, CONFIG1);
			if (!shouldSucceed) {
				fail("Should not have been able to store '" + confName1
						+ "' to '" + coll1 + "'\n\tand then '" + confName2
						+ "' to '" + coll2 + "'");
			}
			...
			if (shouldSucceed) {
				fail("Should have been able to store '" + confName1 + "' to '"
						+ coll1 + "'\n\tand then '" + confName2 + "' to '"
						+ coll2 + "': " + xe.getMessage());
			}
		}
	}

	private void storeConfiguration(XmldbURI collPath, XmldbURI confName,
			String confContent) throws XMLDBException {
		String fullCollPath = URI + collPath.toString();
		System.out.println("Storing configuration '" + confName + "' to '"
				+ collPath + "'");
		Collection configColl = DatabaseManager.getCollection(fullCollPath,
				"admin", null);
		if (configColl == null) {
			CollectionManagementService cms = (CollectionManagementService) testCollection
					.getService("CollectionManagementService", "1.0");
		...
		Resource res = configColl.createResource(confName.toString(),
				"XMLResource");
		assertNotNull(res);
