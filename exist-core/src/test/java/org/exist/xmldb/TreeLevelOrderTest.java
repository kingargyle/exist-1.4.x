...
public class TreeLevelOrderTest extends AbstractDBTest {
	...
	static {
		try {
			getTextContentMethod = Node.class.getMethod("getTextContent",
					(Class[]) null);
		} catch (Exception ex) {
		}
	}
	...
	public void testTreeLevelOrder() {
		...
		try {
			...
			xmlDocument
					.append("<from><![CDATA[tobias.wunden@o2it.ch]]></from>");
	...
	private final void store(String xml, XQueryService service, String document) {
		try {
			...
			query.append("let $isLoggedIn := xdb:login('" + eXistUrl
					+ DBBroker.ROOT_COLLECTION + "', 'admin', 'admin'),");
			query.append("$doc := xdb:store(\"" + eXistUrl
					+ DBBroker.ROOT_COLLECTION + "\", $document, $survey)");
	...
	/**
	 * Loads the xml document identified by <code>document</code> from the
	 * database.
	 * 
	 * @param service
	 *            the xquery service
	...
	private final Node load(XQueryService service, String document)
		...
		StringBuffer query = new StringBuffer();
		query.append("xquery version \"1.0\";");
		query.append("let $survey := xmldb:document(concat('"
				+ DBBroker.ROOT_COLLECTION + "', $document))");
		...
		if (set != null && set.getSize() > 0) {
			return ((XMLResource) set.getIterator().nextResource())
					.getContentAsDOM();
		}
		return null;
	}
	...
	/**
	 * Retrieves the base collection and thereof returns a reference to the
	 * collection's xquery service.
	 * 
	 * @param db
	 *            the database
	...
	private final XQueryService getXQueryService(Database db) {
		try {
			Collection collection = DatabaseManager.getCollection(eXistUrl
					+ DBBroker.ROOT_COLLECTION, "admin", "");
			if (collection != null) {
