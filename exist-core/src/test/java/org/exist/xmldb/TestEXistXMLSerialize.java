...
public class TestEXistXMLSerialize extends TestCase {
	...
	private final static String XML_DATA = "<test>"
			+ "<para>ääööüüÄÄÖÖÜÜßß</para>" + "<para>\uC5F4\uB2E8\uACC4</para>"
			+ "</test>";

	private final static String XSL_DATA = "<xsl:stylesheet xmlns:xsl=\"http://www.w3.org/1999/XSL/Transform\" "
			+ "version=\"1.0\">"
	...
	public void setUp() throws Exception {
		testFile = new File(ClassLoader.getSystemClassLoader()
				.getResource("org/exist/xmldb/PerformanceTest.xml").toURI()
				.toURL().getFile());
		Class cl = Class.forName("org.exist.xmldb.DatabaseImpl");
		database = (Database) cl.newInstance();
		database.setProperty("create-database", "true");
		DatabaseManager.registerDatabase(database);
		c = DatabaseManager.getCollection("xmldb:exist://"
	...
	public void testSerialize1() throws Exception {
		...
		XMLResource resource = (XMLResource) c.createResource(null,
				"XMLResource");
	...
	public void testSerialize4() throws Exception {
		...
		Node node = resource.getContentAsDOM();
		System.out
				.println("Attempting serialization using eXist's SAX serializer");
		StringWriter writer = new StringWriter();
		Properties outputProperties = new Properties();
		outputProperties.setProperty("indent", "yes");
	...
	public void testSerialize5() throws Exception {
		XMLResource resource = (XMLResource) c.createResource("test.xml",
				"XMLResource");
		resource.setContent(XML_DATA);
		System.out.println("Storing resource: " + resource.getId());
		c.storeResource(resource);

		XMLResource style = (XMLResource) c.createResource("test.xsl",
				"XMLResource");
