...
public class FtQueryTest extends AbstractDBTest {

	private static String COLLECTION_CONFIG1 = "<collection xmlns=\"http://exist-db.org/collection-config/1.0\">"
			+ "	<index>"
			+ "		<fulltext default=\"all\">"
			+ "		</fulltext>"
			+ "	</index>" + "</collection>";

	private final static String TEST_XML = "<test-doc>"
	...
	private final static String QNAME_XML = "<root>" + "   <test>"
			+ "       <node id=\"1\">First node</node>"
			+ "       <node id=\"2\">Second node</node>" + "   </test>"
			+ "</root>";

	private final static String NESTED_XML = "<root>\n"
			+ "    <nested><s>un</s>even</nested>\n"
			+ "    <nested>un<s>suitable</s></nested>\n"
			+ "    <nested><s>in</s><s>ap</s><s>pro</s><s>pri</s><s>ate</s></nested>\n"
			+ "</root>";

	private final static String MATCH_COUNT = "<doc> term term <level1>term term</level1><level1>term<level2>term</level2></level1></doc>";

	private final static String FILES[] = { "hamlet.xml", "macbeth.xml",
			"r_and_j.xml" };
	...
	private static File SHAKES_DIR;
	private static File MODS_DIR;

	private static final String TEST_COLLECTION_NAME = "testft";
	private static final String TEST_COLLECTION_PATH = DBBroker.ROOT_COLLECTION
	...
	public void testFtOperators() {
		try {
			...
			// assertXMLEqual(result.getResource(0).getContent().toString(),
			// "<LINE>Fillet of a <exist:match xmlns:exist='http://exist.sourceforge.net/NS/exist'>fenny</exist:match> <exist:match xmlns:exist='http://exist.sourceforge.net/NS/exist'>snake</exist:match>,</LINE>");
			...
			result = service
					.query("declare namespace mods='http://www.loc.gov/mods/v3'; //mods:titleInfo[mods:title &= 'self*']");
			assertEquals(2, result.getSize());
			result = service
					.query("declare namespace mods='http://www.loc.gov/mods/v3'; //mods:titleInfo[mods:title &= 'self employed']");
			assertEquals(1, result.getSize());
			result = service
					.query("declare namespace mods='http://www.loc.gov/mods/v3'; //mods:titleInfo[match-all(mods:title, '.*ploy.*')]");
	...
	public void testFtMatchFunctions() {
		try {
			...
			ResourceSet result = service
					.query("//SPEECH[match-all(LINE, 'love')]");
			assertEquals(160, result.getSize());
			...
			result = service
					.query("//SPEECH[match-all(LINE, 'fenny', 'snake')]/LINE[1]");
			assertEquals(1, result.getSize());
			assertXMLEqual(result.getResource(0).getContent().toString(),
					"<LINE>Fillet of a fenny snake,</LINE>");
			// assertXMLEqual(result.getResource(0).getContent().toString(),
			...
			result = service
					.query("//SPEECH[text:match-all(LINE, ('fenny', 'snake'))]/LINE[1]");
			assertEquals(1, result.getSize());
			assertXMLEqual(result.getResource(0).getContent().toString(),
					"<LINE>Fillet of a fenny snake,</LINE>");
			// assertXMLEqual(result.getResource(0).getContent().toString(),
			...
			assertEquals(90, result.getSize());
			result = service
					.query("declare namespace mods='http://www.loc.gov/mods/v3'; //mods:titleInfo[text:match-all(mods:title, 'self.*')]");
			assertEquals(2, result.getSize());
			result = service
					.query("declare namespace mods='http://www.loc.gov/mods/v3'; //mods:titleInfo[text:match-all(mods:title, ('self', 'employed'))]");
			assertEquals(1, result.getSize());
			result = service
					.query("declare namespace mods='http://www.loc.gov/mods/v3'; //mods:titleInfo[text:match-all(mods:title, '.*ploy.*')]");
			assertEquals(3, result.getSize());
			...
			// result =
			// service.query("declare namespace mods='http://www.loc.gov/mods/v3'; "
			// +
	...
	public void testFtScan() {
		try {
			...
			String query = queryBody
					+ "t:index-terms(collection('"
					+ TEST_COLLECTION_PATH
					+ "'), \'is\', util:function(xs:QName(\'f:term-callback\'), 2), 1000)";
			...
			query = queryBody
					+ "t:index-terms(collection('"
					+ TEST_COLLECTION_PATH
					+ "')//LINE, \'is\', util:function(xs:QName(\'f:term-callback\'), 2), 1000)";
			...
			query = queryBody
					+ "t:index-terms(collection('"
					+ TEST_COLLECTION_PATH
					+ "')//mods:title, \'s\', util:function(xs:QName(\'f:term-callback\'), 2), 1000)";
			result = service.query(query);
	...
	public void testFtUpdate() {
		try {
			...
			XQueryService service = (XQueryService) testCollection.getService(
					"XQueryService", "1.0");
			service.query("update insert <SPEAKER>First Witch</SPEAKER> preceding //SPEECH[LINE &= 'fenny snake']/SPEAKER");
			ResourceSet result = service
					.query("//SPEECH[LINE &= 'fenny snake']/SPEAKER");
			assertEquals(2, result.getSize());
			result = service
					.query("//SPEECH[LINE &= 'fenny snake' and SPEAKER &= 'first']");
			assertEquals(1, result.getSize());

			service.query("update delete //SPEECH[LINE &= 'fenny snake']/SPEAKER[2]");
			result = service
					.query("//SPEECH[LINE &= 'fenny snake' and SPEAKER &= 'first']");
			assertEquals(1, result.getSize());
		} catch (XMLDBException e) {
	...
	public void testFtConfiguration() {
		...
		try {
			...
			String config = "<collection xmlns=\"http://exist-db.org/collection-config/1.0\">"
					+ "    <index>"
			...
			config = "<collection xmlns=\"http://exist-db.org/collection-config/1.0\">"
					+ "   <index>"
	...
	public void testQNameConfiguration() {
		...
		try {
			...
			doQuery(service,
					"update insert <node id='3'>Third node</node> following //test/node[last()]",
					0);
			...
			doQuery(service,
					"update insert <node id='4'>Fourth <nested>node</nested></node> following //test/node[last()]",
					0);
			...
			doQuery(service,
					"for $i in 1 to 100 return"
							+ "   update insert <node id='i{$i}'>Inserted node</node> preceding //test/node[1]",
					0);
	...
	public void testFtQNameScan() {
		try {
			...
			String query = queryBody
					+ "t:index-terms(collection('"
					+ TEST_COLLECTION_PATH
					+ "')//node, (), util:function(xs:QName(\'f:term-callback\'), 2), 1000)";
			ResourceSet result = service.query(query);
	...
	public void testReindex() {
		try {
			...
			IndexQueryService mgmt = (IndexQueryService) testCollection
					.getService("IndexQueryService", "1.0");
	...
	public void bugtestMatchCount() {
		...
		try {
			...
			assertEquals("doc(6)", result.getResource(0).getContent()
					.toString());
			assertEquals("level1(2)", result.getResource(1).getContent()
					.toString());
			assertEquals("level1(2)", result.getResource(2).getContent()
					.toString());
			assertEquals("level2(1)", result.getResource(3).getContent()
					.toString());
		} catch (Exception e) {
			e.printStackTrace();
			fail(e.getMessage());
	...
	private void doQuery(XQueryService service, String query, int expected)
	...
	public void setUp() throws Exception {
		try {
			SHAKES_DIR = new File(ClassLoader.getSystemClassLoader()
					.getResource("samples/shakespeare").toURI().toURL()
					.getFile());
			MODS_DIR = new File(ClassLoader.getSystemClassLoader()
					.getResource("samples/mods").toURI().toURL().getFile());

			// Since we use the deprecated fn:match-all() function, we have to
			// be sure is is enabled
			Configuration config = new Configuration();
			config.setProperty(
					FunctionFactory.PROPERTY_DISABLE_DEPRECATED_FUNCTIONS,
					new Boolean(false));
			...
			for (int i = 0; i < modsFiles.length; i++) {
				if (modsFiles[i].isFile()) {
					XMLResource doc = (XMLResource) testCollection
							.createResource(modsFiles[i].getName(),
									"XMLResource");
					doc.setContent(modsFiles[i]);
					testCollection.storeResource(doc);
					assertNotNull(testCollection.getResource(modsFiles[i]
							.getName()));
				}
			}
